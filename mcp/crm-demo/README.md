...
## 4. 本番化（Supabaseへ差し替え）— 実装済み・環境変数で自動切替

`server.mjs` は **環境変数の有無でデータ源を自動切替**します（`§SUPABASE` / `LIVE_HANDLERS` / `callRpc` 参照）。
**ツール名・入出力・操作方法・read-only は不変**なので、Claude側の使い方はそのまま（C-6）。

- 未設定 → **デモデータ**（既定・従来どおり）。
- `SUPABASE_URL` と 読み取り専用キーを設定 → 各ツールが **PostgREST の `/rest/v1/rpc/<fn>`** を叩く。
  LIVE時にRPCが失敗したらエラーを返します（デモ値で誤魔化さない）。

`.mcp.json` の `crm-demo.env` に設定する環境変数:

```jsonc
"env": {
  "SUPABASE_URL": "https://<project-ref>.supabase.co",
  // 読み取り専用キー。優先順: SUPABASE_MCP_KEY > SUPABASE_READONLY_KEY > SUPABASE_ANON_KEY
  "SUPABASE_MCP_KEY": "<read-only key / service-account token>"
}
```

対応するRPC（10ツール→10関数）: `search_companies` / `get_company` / `list_contacts` / `find_matches` /
`suggest_matches` / `list_tags` / `get_masters` / `get_newsletter_segment` / `search_documents` / `get_company_timeline`。

**認可（SEC-X3/X6）**: `service_role` は使いません。DB側で**この10関数のみに EXECUTE を付与した読み取り専用ロール**を用意し、
RLSで read を制限したうえで、そのロールのキー/トークンを上記に設定します。鍵は `process.env`（ローカル）/
Cloudflare Secrets（リモート）から読み、**Claudeには渡しません**。
