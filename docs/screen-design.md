...
| S-15 | 期限・タスク（課題管理） | `/schedule`, `/schedule/new`, `/schedule/[id]`, `/schedule/[id]/edit` | 1.5 | staff | 税務期限＋所内業務を親子課題で管理（4ビュー） |
...
### S-15 期限・タスク（課題管理 v2 — Jira/Redmine風）
- **正典UI**: `mockups/schedule.html` / `task-form.html` / `task-detail.html`（実装済み: `app/schedule/`）。
- **課題の構造**: 親課題＋子課題の2階層。`scope` で **顧客の課題**（企業に紐づく）と **所内の課題**（大吉会計の業務、印「大」）を区分。親課題の進捗（％）は子課題から**読み取り時に自動集計**（保存しない）。
- **4ビュー**（同一データ・画面内切替）:
  - 一覧: 期日順ツリー（▾で子課題開閉・丸チェック完了・行 ⋯ メニュー=子課題追加/編集/完了/担当変更/期日変更/削除）。
  - ボード: 親課題ごとのレーン＋「単独の課題」レーン。カードを 未対応→対応中→完了 へ**ドラッグでステータス変更**。
  - ガントチャート: 今日を含む47日窓。親課題=集約バー、期間バー＋進捗、期日のみ=マイルストーン（◆）、超過は赤ハッチ＋「◯日 超過」。
  - カレンダー: 月表示（前月/翌月/今日）。期日・開始日をイベント表示、完了は打ち消し、遅延は赤マーク。
- **スコープタブ**: すべて / 顧客の課題 / 所内の課題（件数付き・全ビュー共通）。担当・種別・未完了のみ・キーワード(q)の絞り込みは URL クエリ。
- **課題作成/編集**: 顧客の課題は企業を検索・選択するまで他項目をロック。親課題・関連議事録・資料は選択企業（またはスコープ=所内）のものだけに絞る（`app_task_form_lookup`）。自動生成課題は題名・種別・日付を編集不可（担当・状態・進捗・説明は可）。
- **課題詳細**: 説明 / 子課題（チェックで完了⇄取消） / コメント投稿 / 変更履歴（状態・担当・期日・進捗の変更を自動記録） / 詳細メタ / 操作（子課題追加・削除=手動のみ・子課題ごと論理削除）。
- **使い方ツアー**: スポットライト＋コーチカード（`components/tour.ts`）。トップバー「使い方」から6ステップ。ボードビューの**新機能告知は初回のみ**（localStorage）。
- **DB/RPC**: `schedule_items`（parent_id / scope / start_date / progress / description 追加）＋ `schedule_comments`（comment/event）。`app_list_schedule`（集計付き一覧）・`app_get_task`・`create_task`・`update_schedule_item`・`complete_schedule_item`・`delete_task`・`add_task_comment`。マイグレーション: `supabase/migrations/0008_tasks_rpc.sql`。

### 共通UI
- **案内人ツアー（使い方）**: 全画面の「使い方」ボタンはスポットライト・ツアー（`components/tour.ts`＋`TourButton`）。紺のベールで沈め、対象を金の印章リングで照らし、金の印「案」のコーチカードでステップ案内（ドット進捗・←/→・Esc）。各画面がステップ（sel/title/body）を定義。静的ヘルプモーダル（旧GuideButton）は廃止。
