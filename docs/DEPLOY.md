...
2. **マイグレーション適用**: `supabase/migrations/0001_init.sql`〜`0012_features_rpc.sql` を順に実行（Supabase SQL Editor に貼り付け、または `supabase db push`）。`_pglite_bootstrap.sql` は**適用しない**（Supabaseは auth スキーマ/ロールを標準装備）。※既存本番に `0001`〜`0011` 適用済みなら**差分 `0012` のみ**適用。
...
8. **Supabase Auth 連携**（アプリ層は実装済み）: `lib/auth/session.ts` の supabase 分岐・`app/actions/auth.ts` の `supabaseSignIn`（`signInWithPassword`）・`lib/data/supabase-server.ts` の `@supabase/ssr` Cookie 受け渡しは**実装済み**。本番では Supabase → Authentication → URL Configuration に本番URLを登録し、`scripts/create-admin.mjs` で `app_users.id` = `auth.users.id` の管理者を作成して疎通確認する。
